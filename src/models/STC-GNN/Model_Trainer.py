...
import numpy as np
import torch
from Metrics import ModelEvaluator, mask_data
...
class ModelTrainer(object):
    def __init__(self, params: dict, data: dict):
        ...
        self.prior_graph = [
            torch.from_numpy(data["s_adj"]).float().to(self.params["device"]),
        ...
        self.optimizer = optim.Adam(
            params=self.model.parameters(), lr=self.params["learn_rate"], weight_decay=self.params["decay_rate"]
        )

    def get_model(self):
        if self.params["model"] == "STC-GNN":
            model = STCGNN(
                num_nodes=self.params["H"] * self.params["W"],
    ...
    def train(self, data_loader: dict, modes: list, early_stop_patience=10):
        ...
        for epoch in range(1, 1 + self.params["num_epochs"]):
            ...
            # epoch end
            log = (
                f'Epoch {epoch}: training time: {run_time["train"][-1]:.4} s/epoch, training loss: {loss_curve["train"][-1]:.4}; '
            ...
            if loss_curve["validate"][-1] < val_loss:
                ...
                torch.save(
                    checkpoint, self.params["output_dir"] + f'/{self.params["model"]}-{self.params["time_slice"]}.pkl'
                )
                patience_count = early_stop_patience
            else:
    ...
    def test(self, data_loader: dict, modes: list):
        ...
        trained_checkpoint = torch.load(
            self.params["output_dir"] + f'/{self.params["model"]}-{self.params["time_slice"]}.pkl'
        )
        print(
            f'Successfully loaded trained {self.params["model"]} model - epoch: {trained_checkpoint["epoch"]}, training loss: {trained_checkpoint["train_loss"]}, validation loss: {trained_checkpoint["val_loss"]}'
        )
        ...
        for mode in modes:
            ...
            model_evaluator.evaluate_binary(
                y_pred_prob=forecast_masked, y_true_bi=ground_truth_masked, threshold=self.threshold, mode=mode
            )
