...
def mask_data(x: np.array, H: int, W: int, mask):  # for evaluation only on selected grids
    assert (len(x.shape) == 4) | (
        len(x.shape) == 5
    )  # graph: (num_samples, horizon, N, C) / grid: (num_samples, horizon, C, H, W)
...
class ModelEvaluator(object):
    ...
    def one_step_eval_bi(self, step_y_pred_prob: np.array, step_y_true_bi: np.array, threshold: list):
        ...
        macro_f2 = (
            (1 + self.beta**2)
            * sum(tp_lst)
            / ((1 + self.beta**2) * sum(tp_lst) + (self.beta**2) * sum(fn_lst) + sum(fp_lst))
        )
        ...
        step_y_pred_prob, step_y_pred_bi, step_y_true_bi = (
            step_y_pred_prob.flatten(),
            step_y_pred_bi.flatten(),
            step_y_true_bi.flatten(),
        )
